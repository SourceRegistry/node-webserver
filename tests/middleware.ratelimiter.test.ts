...
import {fixedWindowLimit, slidingWindowLimit} from "../src/middlewares/ratelimiter";
import {MemoryStore} from "../src/middlewares/ratelimiter/InMemory";
import {SlidingWindowStore} from "../src/middlewares/ratelimiter/SlidingWindow";
...
describe("sliding window rate limiter", () => {
    it("injects headers without recursive setHeaders calls", async () => {
        const middleware = slidingWindowLimit({max: 2});
        const captured: Record<string, string>[] = [];
    ...
    it("returns a limit response with JSON body and callback info", async () => {
        ...
        const hits: Array<{ current: number; max: number; key: string; remaining: number; reset: number; resetTimeMs: number }> = [];
        const middleware = slidingWindowLimit({
            max: 2,
            message: {message: "slow down"},
            onRateLimit: (_event, info) => {
                hits.push(info);
...
describe("sliding window rate limiter memory store", () => {
    it("increments, resets, and clears keys", async () => {
        const store = new SlidingWindowStore({windowMs: 5});

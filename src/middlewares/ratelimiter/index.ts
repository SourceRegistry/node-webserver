...
export type Options = {
     ...
     */
    onRateLimit?: (event: RequestEvent, info: {
        current: number;
        max: number;
        key: string;
        reset: number;
 ...
 */
export function slidingWindowLimit(options: Options): Middleware {
    const {
        windowMs = 60_000,
        max,
    ...
    return async (event, next) => {
        ...
        if (current > max) {
            if (onRateLimit) {
                const resetTimeMs = (reset - Date.now());
                onRateLimit(event, { current, max, key: rateLimitKey, reset: Math.floor(reset / 1000), remaining: 0, resetTimeMs });
            }
